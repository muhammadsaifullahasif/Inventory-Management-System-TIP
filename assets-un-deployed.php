										...
										<table id="example1" class="table table-bordered table-striped table-sm" style="max-width: 100%;">
											...
											<tbody id="display_ready_to_deploy_assets">
												<?php
													...
													$query = mysqli_query($conn, "SELECT id, user_id, igr_number, asset_name, asset_type, asset_category, item_type, item_size, SUM(qty) as qty FROM assets WHERE active_status='1' && delete_status='0' && parent_id is NULL GROUP BY asset_category ORDER BY qty DESC");
						...
						$.ajax({
							...
							success: function(result) {
								if(result == 2) {
									...
									// display_assets();
									$("#example1").load("assets-un-deployed.php #display_ready_to_deploy_assets");
								} else if(result == 1) {
