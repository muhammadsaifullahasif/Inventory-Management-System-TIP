										...
										<table id="example1" class="display table table-bordered table-striped table-sm" style="max-width: 100%;">
											<thead>
												<tr>
													...
													<?php if(is_superadmin() || is_admin()) { echo "<th>Action</th>"; } ?>
												</tr>
											</thead>
											<tbody id="display_assets">
												...
												// $query = mysqli_query($conn, "SELECT * FROM assets WHERE active_status='1' && delete_status='0' && parent_id is NULL ORDER BY qty DESC");

												$query = mysqli_query($conn, "SELECT id, user_id, igr_number, asset_name, asset_type, asset_category, item_type, item_size, SUM(qty) as qty FROM assets WHERE active_status='1' && delete_status='0' && parent_id is NULL GROUP BY asset_category, item_type, item_size, asset_name ORDER BY qty DESC");
												...
												if(mysqli_num_rows($query) > 0) {
													...
													while($result = mysqli_fetch_assoc($query)) {
														...
														// if($result['asset_type'] == 1) {
															// echo "Computer / Desktop / Server";
														// } else {
															echo $result['asset_name'];
														// }
														echo "</td>";
						...
						$.ajax({
							...
							success: function(result) {
								if(result == 2) {
									...
									// display_assets();
									$("#example1").load("assets.php #display_assets");
								} else if(result == 1) {
