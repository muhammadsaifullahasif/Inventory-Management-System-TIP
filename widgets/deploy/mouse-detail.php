...
if(mysqli_num_rows($query) > 0) {
	...
	$accessories_query = mysqli_query($conn, "SELECT * FROM accessories WHERE item_category='7' && active_status='1' && delete_status='0' && qty!='0'");
	...
	if(mysqli_num_rows($accessories_query) > 0) {
		...
		<div class="mb-3">
			<label>Mouse:</label>
			<select class="form-control form-control-border" id="mouse" name="asset_id" required>
		...
		echo "<p class='alert alert-danger'>You haven't any Mouse in Inventory</p>";
	}
