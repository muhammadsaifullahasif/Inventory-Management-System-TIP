...
if(mysqli_num_rows($query) > 0) {
	...
	$accessories_query = mysqli_query($conn, "SELECT * FROM accessories WHERE item_category='8' && active_status='1' && delete_status='0' && qty!='0'");
	...
	if(mysqli_num_rows($accessories_query) > 0) {
		...
		<div class="mb-3">
			<label>Speaker:</label>
			<select class="form-control form-control-border" id="speaker" name="asset_id" required>
		...
		echo "<p class='alert alert-danger'>You haven't any Speaker in Inventory</p>";
	}
