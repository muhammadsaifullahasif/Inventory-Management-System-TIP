...
if(mysqli_num_rows($query) > 0) {
	...
	$accessories_query = mysqli_query($conn, "SELECT * FROM accessories WHERE item_category='11' && active_status='1' && delete_status='0' && qty!='0'");
	...
	if(mysqli_num_rows($accessories_query) > 0) {
		...
		?>
		<div class="mb-3">
			<label>Network HUB:</label>
			<select class="form-control form-control-border" id="network_hub" name="asset_id" required>
				<option value="">Select Network HUB</option>
				<?php
		...
		echo "<p class='alert alert-danger'>You haven't any Network HUB in Inventory</p>";
	}
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#network_hub').on('change', function(){
