...
if(mysqli_num_rows($query) > 0) {
	...
	$accessories_query = mysqli_query($conn, "SELECT * FROM accessories WHERE item_category='10' && active_status='1' && delete_status='0' && qty!='0'");
	...
	if(mysqli_num_rows($accessories_query) > 0) {
		...
		<div class="mb-3">
			...
			<select class="form-control form-control-border" id="network_switch" name="asset_id" required>
		...
		echo "<p class='alert alert-danger'>You haven't any Network Switch in Inventory</p>";
	}
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$('#network_switch').on('change', function(){
